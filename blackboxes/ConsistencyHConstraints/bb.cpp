...
const double LB[] = { 0.1 , 0.75 , 0.75 , 0.1 , 0.01 , 30000 ,
    1.4 , 2.5 , 40 ,  500,  0.5, 40000, 0.0 };
const double RANGE[] = { 0.3 , 0.50 , 0.50 , 0.9 , 0.08 , 30000 ,
    0.4 , 6.0 , 30 , 1000, 1.0, 30000, 5.0 };

// Expressions coded from Sobieszczanski-Sobieski, Agte and Sandusky, BLISS NASA technical report.

int allDisciplinesOnce ( double   inputs[13] ,
         double   Str   [5 ] ,
         double & Apg        ,
         double & SFC        ,
         double & ESF        ,
...
void display_inputs ( double inputs[13] );

bool read_values ( const string & file_name , double x [13] );
...
int main ( int argc , char ** argv )
{
    ...
    // read inputs and RESCALE them.
    if (!read_values(argv[1],inputs))
        return 1;
    
    // Scaled coupling variables
    double tESF = inputs[10];
    double tWt = inputs[11];
    double tTheta = inputs[12];

    // output variables for MDA (last ones for the consistency equality constraints):
    double Str[5] , Apg , SFC , ESF , Tb , TbUA , Temp , Range, delta_ESF, delta_Wt, delta_Theta;


    int it = allDisciplinesOnce ( inputs , Str , Apg , SFC , ESF , Tb , TbUA , Temp , Range, delta_ESF, delta_Wt, delta_Theta );

    // G7 and G8 constraint should always be satisfied if variables respect their bounds
    // output display :
    if (display)
    {
        cout << "  it = " << it            << endl     // it
...
void display_inputs ( double inputs[13] )
{
    ...
    << "x9 :   Sref = " << inputs[9] << endl
    << "x10:   tESF = "  << inputs[10] << endl
    << "x11:    tWt = "  << inputs[11] << endl
    << "x12: tTheta = "  << inputs[12] << endl<< endl;
}
                ...
                double & Apg           ) const
{
    ...
    double ESF    = inputs[10];
    double Wt     = inputs[11];
    double theta  = inputs[12];
...
bool read_values ( const string & file_name , double x [13] )
{
    ...
    // read and scale from [0;100] to [lb;ub]:
    double v;
    for ( int i = 0 ; i < 13 ; i++ )
    {
        in >> v;
...
/*------------------------------------------------------------------------------*/
/*          MDA analysis (for the consistency equality constraints method)      */
/*------------------------------------------------------------------------------*/
int allDisciplinesOnce ( double   inputs[13] ,
              double   Str   [5 ] ,
              double & Apg        ,
                                ...
                                ) const
{
    ...
    // PROP
    Prop ( inputs , D , SFC , We , ESF , Tb , TbUA , Temp );
    
    // PROP
    Struc ( inputs , We , L , Wt , Wf , theta , Str );
    ...
    // NOTE: the Breguet range uses tWt instead of Wt obtained from Struc.
